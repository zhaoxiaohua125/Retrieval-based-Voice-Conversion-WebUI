...
## 会话总结 - 2026-08-20（修复切说话偶发回音）

- **会话主要目的**: 从 AI 切到普通/混响说话时偶尔出现回音
- **完成的主要任务**: 模式切换先更新 playback_mode 再清 ring；任意模式切换均清 output/monitor 缓冲；切换后约 3 个音频块静音过渡
- **关键决策与解决方案**: 旧逻辑先清缓冲后改模式，回调仍可能按 AI 模式写入残载人声；竞态导致偶发叠音像回音
- **修改的文件列表**: app/audio/stream_manager.py、README.md

---

## 会话总结 - 2026-08-20（普通说话延迟人声回音）

- **会话主要目的**: 启动后直接普通说话，约 2 秒后出现较小延迟人声回音（非切换引起）
- **完成的主要任务**: 说话模式直播直通 outdata（不经 output_ring）；启动/切模式清缓冲；`_cancel_talk_loopback` 抵消 VM 回灌；采集 auto 优先 Aux Output 勿选 B1
- **关键决策与解决方案**: 典型为 AUX 误勾 B2 或 B1 采到直播混音形成 ~2s 反馈环；软件侧减环延迟并做延迟相关消除
- **修改的文件列表**: app/audio/stream_manager.py、app/audio/devices.py、app/ui/settings_dialog.py、README.md

---

## 会话总结 - 2026-08-20（回音问题用户确认已消除）

- **会话主要目的**: 用户反馈 AUX 仅勾 B1、未勾 B2，暂未再发现普通说话延迟回音
...
## 会话总结 - 2026-08-22（联调定稿写入开发大纲）

- **会话主要目的**: 暂无更优自动方案，以手动切换 VAIO3 的 B1 为实操方案；写入开发大纲备查
- **完成的主要任务**: 更新 开发大纲.md 双路监听章节（2026-08-22 定稿）：条带分工、VAIO3 B1 场景表、限制说明、联调备注表
- **关键决策与解决方案**: 日常 VAIO3 只 A1；酷狗进直播手动勾 B1 后停客户端伴奏；可选 Potato Macro 一键切换
- **修改的文件列表**: 开发大纲.md、README.md

---

## 会话总结 - 2026-08-22（VAIO3 B1 半自动方案写入开发大纲）

- **会话主要目的**: 明确 VM Remote API（非8989）、后台进程必要性；将 Phase1 切模式关 B1 + Phase2 pycaw 开 B1 计划写入开发大纲
- **完成的主要任务**: 开发大纲增补 Remote API 说明、B1 状态机、常见场景表、普通说话+伴奏暂停+酷狗聊天影响分析、计划配置项与模块
- **关键决策与解决方案**: 检测用 IPC/内部状态+pycaw，执行用 VM DLL；client_blocks_b1 优先；暂停伴奏时允许酷狗开 B1；暂不单独后台进程
- **修改的文件列表**: 开发大纲.md、README.md
