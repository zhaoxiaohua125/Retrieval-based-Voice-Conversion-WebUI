...
echo.
set /p PYD=Compile app/ to pyd? [y/N]:
set "PYD_FLAG="
if /i "%PYD%"=="y" set "PYD_FLAG=-Pyd"
echo.
...
exit /b 1
:cu118
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0scripts\build_client_package.ps1" -CudaVariant cu118 -CondaPack -CondaBase "F:\zxh\anaconda3" %PYD_FLAG% %*
if errorlevel 1 (echo BUILD FAILED) else (echo BUILD OK: dist\RVC-Client-0.1.0-demo-cu118)
goto end
:cu128
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0scripts\build_client_package.ps1" -CudaVariant cu128 -CondaPack -CondaBase "F:\zxh\anaconda3" %PYD_FLAG% %*
