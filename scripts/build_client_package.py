...
LAUNCHER_BAT = ROOT / 'packaging' / 'launcher' / '启动声迹客户端.bat'
...
def write_launcher(out_dir: Path):
    ...
    bat = (
        ...
        'set "PY="\r\n'
        'set "PYW="\r\n'
        'for /f "delims=" %%i in (\'powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0scripts\\resolve_launch_python.ps1"\') do set "PY=%%i"\r\n'
        'if not defined PY (\r\n'
        ...
        'if exist "%PYW%" (\r\n'
        '  start "" /B "%PYW%" scripts\\run_ui_skeleton.py\r\n'
        '  exit /b 0\r\n'
        ')\r\n'
